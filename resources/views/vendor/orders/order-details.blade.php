...
@section('main-content')
    <div class="mt-5 block-header d-flex justify-content-between">
        ...
        <select name="" class="theme-bg" data-id="{{ $orders[0]->id }}" id="selectStatus">
            <option class="text-light" value="pending" {{ $orders[0]->vendor_status == 'Pending' ? 'selected' : '' }}>
                Pending</option>
            <option class="text-light" value="On Delivery"
                {{ $orders[0]->vendor_status == 'On Delivery' ? 'selected' : '' }}>On Delivery</option>
            <option class="text-light" value="Declined" {{ $orders[0]->vendor_status == 'Declined' ? 'selected' : '' }}>
                Declined</option>
            <option class="text-light" value="Completed" {{ $orders[0]->vendor_status == 'Completed' ? 'selected' : '' }}>
                Completed</option>
        </select>
        <!-- --------------- -->
    </div>
                            ...
                            <table class="table">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <th width="10%">:</th>
                                        <td width="45%"><span
                                                class='badge badge-success'>{{ $orders[0]->order->status }}</span></td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                        <div class="footer-area">
                            <a href="{{ route('vendor.invoice', $orders[0]->id) }}" class="mybtn1"><i
                                    class="fa fa-eye"></i>
                                View Invoice</a>
                        </div>
                    </div>
                                ...
                                <tbody>
                                    @foreach ($orders as $item)
                                        <tr>
                                            ...
                                            <td>
                                                <a target="_blank"
                                                    href="{{ route('shop.product', [$item->vendor->id, Str::slug($item->vendor->shop_name)]) }}">{{ $item->vendor->shop_name }}</a>
                                            </td>
                                            <td>
                                                <span class="badge badge-success">{{ $item->vendor_status }}</span>
                                            </td>
                                            <td>
                                                <a target="_blank"
                                            ...
                                            <td> {{ $item->product->categories->commision }} % =
                                                {{ $item->qty * $item->product->price * ($item->product->categories->commision / 100) }}
                                            </td>
                                            <td>
